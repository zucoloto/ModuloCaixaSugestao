...
public class UsuarioRepositoryJPA implements UsuarioRepository, Serializable {
	...
	public List<Usuario> listarTodos() {
		return em.createQuery("from Usuario order by nomeUsuario", Usuario.class).getResultList();
	}
	...
	public Usuario buscarPorNome(String nome) {
		...
		try {
			usuario = em.createQuery("from Usuario where upper(nomeUsuario) = :pNome", Usuario.class)
	...
	public Usuario buscarPorIdentidade(String identidade) {
		...
		try {
			usuario = em.createQuery("from Usuario where identidade = :pIdentidade", Usuario.class)
	...
	public Usuario buscarPorNomeSenha(String nome, String senha) {
		try {
			Query query = em.createQuery("from Usuario where upper(nomeUsuario) = :pNome and senha = :pSenha",
					Usuario.class);
